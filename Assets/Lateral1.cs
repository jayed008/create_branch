...
public class Lateral1 : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        if (branchHeight >= 3)    
        {
            ...
            if (branchHeight <= 20)                     //主干多长时侧枝开始生长,多长是不在主要为该侧枝提供营养，半径不在随主干变换
                initRadius = 0.9f * neededRadius;
            if (branchHeight > 20)
                initRadius += 0.0001f;
            ...
            if (totalHeight < 5)                //侧枝长多长
                totalHeight += 0.2f;            //总长变化情况
            if (totalHeight >= 5)
                totalHeight += 0.001f;          //侧枝长度大于指定长度，长度增长变慢
    ...
    public void Initialization()
    {
        ...
        neededVertice = finalVertices[1];
        neededCentre = finalCentres[1] + new Vector3(0, 1, 0) * finalVertices[1].y;
        axis2 = Vector3.Normalize(GetTangentVector(neededCentre, neededVertice));// 着生点的切向量
        axis1 = Vector3.Normalize(neededCentre - neededVertice);                 // 所在节的圆心到着生点的向量
        axis3 = Vector3.up;
        neededRadius = finalRadius[1];

        //transform.position = neededCentre;
        lateralAngle = Mathf.PI / 2.1f;
    }
}
